...
namespace IntecoaAG.XAFExt.CDS.Tests.FunctionalTests
{
    ...
    [RequiresMTA]
    [TestFixture, Description("Заполнение БД данными")]
    public class CreateDBForAnalysisTests : BaseTest
    {
        ...
        protected void FillDatabaseForAnalysis(Session ssn) {
            ...
            IList<fmCSubject> subjectList = new List<fmCSubject>();

            for (int i = 0; i < maxSubjectCount; i++) {
                fmCSubject subject = objectSpace.CreateObject<fmCSubject>();
            ...
            IList<fmCOrder> OrderList = new List<fmCOrder>();

            for (int i = 0; i < maxOrderCount; i++) {
                fmCOrder order = objectSpace.CreateObject<fmCOrder>();
                ...
                order.pSubject = subjectList[GetRandomIntegerFromInterval(1, subjectList.Count) - 1];
